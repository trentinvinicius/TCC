...
{
public:
    RopeCallback(ood::RigidBody** bodies, unsigned int num_bodies):
    m_bodies(bodies),
    m_num_bodies(num_bodies)
    {
        m_interpolator = new ood::CubicVec3Interpolator() ;
    }
...
private:
    ...
    osg::ref_ptr<ood::Vec3Interpolator>  m_interpolator ;

    ood::RigidBody** m_bodies ;
...
main(int argc, char** argv)
{
    ...
    ood::RigidBody*  bodies[NUM_BODIES] ;

    for(unsigned int i=0; i<NUM_BODIES; i++) {
        ood::RigidBody*  body = new ood::RigidBody() ;
        ood::Joint*      ball = new ood::BallJoint() ;
