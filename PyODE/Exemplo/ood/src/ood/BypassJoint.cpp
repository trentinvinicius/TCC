...
BypassJoint::setRelativeRotation( ooReal step_size, const osg::Quat& qrel, int& row, ooReal erp, ooReal cfm, BodyMask mask )
{
    ...
    osg::Quat   qe = osg::Quat(0,0,0,1) ;

    osg::Quat   q1 = m_body1->getQuaternion() ;
    ...
    if( m_body1.valid() ) {
        ...
        if( m_body2.valid() ) {
            ...
            osg::Quat   q2 = m_body2->getQuaternion() * q1.inverse() ;
...
BypassJoint::setRelativePosition( ooReal step_size, const osg::Vec3& prel, int& row, ooReal erp, ooReal cfm, BodyMask mask )
{
    ...
    if( m_body1.valid() ) {
        ...
        if( m_body2.valid() ) {
            ...
            p1 = prel * m_body1->getMatrix() ;

            p2 = m_body2->getPosition() ;


        } else {

            p1 = m_body1->getPosition() ;
            p2 = prel ;
        }
...
{
    PS_DBG2("ood::BypassJoint::cloneODEJoint(%p, world=%p)", this, world) ;
...
{
    PS_DBG2("ood::BypassJoint::finalize(%p)", this) ;
