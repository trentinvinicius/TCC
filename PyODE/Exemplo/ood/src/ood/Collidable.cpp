    ...
    m_last_collision_frame  ( other.m_last_collision_frame )
{
    ...
    m_collision_parameters = osg::clone( other.m_collision_parameters.get(), copyop ) ;
#endif
}
/* ....................................................................... */
/* ======================================================================= */
...
{
    PS_DBG2("ood::Collidable::addToWorldInternal(%p, world=%p)", this, world) ;
    ...
    if( space ) {
        ...
        PS_WARN("ood::Collidable::addToWorldInternal(%p, ...): %p not a Space", this, world) ;
    }
...
{
    PS_DBG2("ood::Collidable::removeFromWorldInternal(%p, world=%p)", this, world) ;
    ...
    if( space ) {
        ...
        PS_WARN("ood::Collidable::removeFromWorldInternal(%p, ...): %p not a Space", this, world) ;
    }
...
/* ....................................................................... */
bool
Collidable::performRayCast( const osg::Vec3& from, const osg::Vec3& to, osg::Vec3& position)
{
    ...
    dGeomID     ray = dCreateRay(NULL, dir.normalize()) ;
     ...
     * [3] ray cast
     */
    const unsigned int  max_contacts = 16 ;

    dContactGeom*       contacts = new dContactGeom[ max_contacts ] ;



    unsigned int        num_contacts = dCollide( ray, getODEGeom(), max_contacts, contacts, sizeof(dContactGeom) ) ;
    ...
    for( unsigned int i=0; i<num_contacts; i++ ) {
        ...
        const dContactGeom&     current = contacts[i] ;


        if( dist < 0.0  ||  dist > current.depth ) {
            dist = current.depth ;

            position.set( current.pos[0], current.pos[1], current.pos[2] ) ;
        }
    }
...
{
    PS_DBG2("ood::Collidable::_cloneODEGeom(src=%p, dest=%p)", src, dst) ;
