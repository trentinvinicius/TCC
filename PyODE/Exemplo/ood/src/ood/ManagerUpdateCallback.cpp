...
ManagerUpdateCallback::operator()(osg::Node* n, osg::NodeVisitor* nv)
{
    ...
    if( ! manager->getDone() ) {
        ...
        if( m_last_s > 0.0  &&  sim_time > m_last_s ) {
            ...
            //! NOTE: sometimes the event buffer is NULL. Why?

            Events* buf = manager->getWorld()->getFrontEventsBuffer() ;


            if( buf ) {
                const Events::ViewMatrixRequests&   view_matrix_requests = buf->getViewMatrixRequests() ;
