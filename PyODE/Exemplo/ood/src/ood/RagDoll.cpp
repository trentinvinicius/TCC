...
/* ....................................................................... */
void
RagDoll::criticalSection(void)
{
    this->Container::criticalSection() ;
...
namespace {
    ...
    {
    public:
        MyBoneUpdate(ood::RigidBody* body, const osg::Matrix& offset, osgAnimation::Skeleton* skeleton):
            m_body      ( body ),
            m_offset    ( offset),
            m_skeleton  ( skeleton) {}
    ...
    private:
        osg::ref_ptr<ood::RigidBody>         m_body ;
        osg::Matrix                             m_offset ;
...
{
    PS_DBG2("ood::RagDoll::_initRagDoll(%p)", this) ;
...

    {
        ...
        for( unsigned int i=0; i< object_list.size(); i++ ) {
            ...
            if( body ) {
                ...
                if( num_children != 0 ) {
                    PS_WARN("ood::RagDoll::_initRagDoll(%p): body \"%s\" has %u children. Removing", this, body->getName().c_str(), num_children) ;
     ...
     */
    m_root_bone = bone_map["Root_Armature"].get() ;
    PS_ASSERT1( m_root_bone.valid() ) ;
    ...
    for( unsigned int i = 0;  i < body_list.size();  i++ ) {
        ...
        osgAnimation::Bone* bone = bone_map[ body->getName() + "_Armature" ] ;

        if( ! bone ) {
            PS_WARN("ood::RagDoll::_initRagDoll(%p): cannot find bone \"%s\"", this, body->getName().c_str()) ;
