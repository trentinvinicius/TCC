...
#if 0
namespace {
static bool checkEventList(const ood::Events& events)
{
    return events.getEventList().size() != 0 ;
}

static bool writeEventList(osgDB::OutputStream& os, const ood::Events& events)
{
    const ood::Events::EventList&    event_list = events.getEventList() ;
...
static bool readEventList(osgDB::InputStream& is, ood::Events& events)
{
    is >> is.BEGIN_BRACKET ;
