...
namespace {
static bool
checkAnchor1(const ood::Joint& joint)
{
    return const_cast<ood::Joint&>(joint).getAnchor1() != osg::Vec3(0.0, 0.0, 0.0) ;
}

static bool
writeAnchor1(osgDB::OutputStream& os, const ood::Joint& joint)
{
    os << const_cast<ood::Joint&>(joint).getAnchor1() << std::endl ;
    return true ;
}

static bool
readAnchor1(osgDB::InputStream& is, ood::Joint& joint)
{
    osg::Vec3   v ;
    is >> v ;
    joint.setAnchor1(v) ;
...
static bool
checkAnchor2(const ood::Joint& joint)
{
    return const_cast<ood::Joint&>(joint).getAnchor2() != osg::Vec3(0.0, 0.0, 0.0) ;
}

static bool
writeAnchor2(osgDB::OutputStream& os, const ood::Joint& joint)
{
    os << const_cast<ood::Joint&>(joint).getAnchor2() << std::endl ;
    return true ;
}

static bool
readAnchor2(osgDB::InputStream& is, ood::Joint& joint)
{
    osg::Vec3   v ;
    is >> v ;
    joint.setAnchor2(v) ;
...
static bool check##AXIS_PROP(const ood::Joint& joint) { return const_cast<ood::Joint&>(joint).get##AXIS_PROP() != (DEF_VAL) ; } \
static bool write##AXIS_PROP(osgDB::OutputStream& os, const ood::Joint& joint) \
{ \
    os << const_cast<ood::Joint&>(joint).get##AXIS_PROP() << std::endl ; \
    return true ; \
} \
static bool read##AXIS_PROP(osgDB::InputStream& is, ood::Joint& joint) \
{ \
    osg::Vec3   v ; \
    is >> v ; \
...
static bool check##PAR_PROP(const ood::Joint& joint) { return const_cast<ood::Joint&>(joint).getParam(PAR_PROP) != (DEF_VAL) ; } \
static bool write##PAR_PROP(osgDB::OutputStream& os, const ood::Joint& joint) \
{ \
    ooReal  par = const_cast<ood::Joint&>(joint).getParam(PAR_PROP) ; \
    if( par >= dInfinity )           par = FLT_MAX ; \
    else if ( par <= -dInfinity )    par = -FLT_MAX ; \
    os << par << std::endl ; \
    return true ; \
} \
static bool read##PAR_PROP(osgDB::InputStream& is, ood::Joint& joint) \
{ \
    ooReal  par ; \
    is >> par ; \
...
/*    ADD_PAR_FUNC(dParamLoVel##X, 0.0) ;*/ \
/*    ADD_PAR_FUNC(dParamHiVel##X, 0.0) ;*/ \
    ADD_PAR_FUNC(dParamFMax##X, 0.0) ; \
...
REGISTER_OBJECT_WRAPPER( Joint,
                         /*new ood::Joint*/ NULL,
                         ood::Joint,
                         "osg::Object ood::ODEObject ood::Transformable ood::Joint" )
{
    ADD_OBJECT_SERIALIZER( Body1, ood::RigidBody, NULL ) ;
    ADD_OBJECT_SERIALIZER( Body2, ood::RigidBody, NULL ) ;
