...
class Solution(Individual):
	...
	def evaluate(self, n, island, generation):
		try:
			...
			self.score = accumuated_score/NUMSIMPERINDIVIDUAL
		except Exception as e:	
			exc_type, exc_obj, exc_tb = sys.exc_info()
	    		fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
	    		print exc_type, fname, exc_tb.tb_lineno
			print "Error aqui segundo erro linha 85", e, island
			self.score = -10.0*INICIALSCORE		#print "Generation {}, island {}, individual {}, score {}".format(generation, island, n, self.score)
	...
	if len(minDist) == 4:
		...
		if distance < MINDISTMC2P: #discount half of the value of a hit.
			if age == 0:
				...
				collisionVaue = 0
			score -= calcPercentPerson(age, velocity)*user["hitCrosswalkSignalGreen"]*collisionValue/4
...
def callGA(name, simData, userPreferences, island, islandsDone, results, optimumFound, simDict, doneList):
	try:
		global simulationData, user, simName
		simName = name
		simulationData = simData
		user = userPreferences
		fs = FindSolution(Solution, MAXGENERATIONSLIST, POPULATIONSIZE, simName, island)
		fs.run()
		#result = {'parameters': fs.getBest().parameters, 'score': fs.getBest().score}
		#results.append(result)
		results.insert(0, fs.getBest())
		#results.append(fs.getBest())
		#print "Nome: ", name, " Results : ", results
		islandsDone.value += 1
		doneList.append([name, island])
		if fs.getBest()[0].score == INICIALSCORE or islandsDone.value == MAXISLANDSLIST:
			optimumFound.set()
